...
*/
	...
	cline++;
	cline++;
	set_title("Other types of errors", &cline, 1);
	set_params(c, "Swap First & Last", &cline, 1, '%', FALSE);
	params[c].is_float = TRUE;
	params[c].is_char = FALSE;
	params[c++].u.fval = &names.swap_names;
	set_params(c, "No middle initial", &cline, 1, '%', FALSE);
	params[c].is_float = TRUE;
	params[c].is_char = FALSE;
	params[c++].u.fval = &names.no_middle_name;
	set_params(c, "Chg. first for initial", &cline, 1, '%', FALSE);
	params[c].is_float = TRUE;
	params[c].is_char = FALSE;
	params[c++].u.fval = &names.fname_for_init;
	set_params(c, "Chg. last name", &cline, 1, '%', FALSE);
	params[c].is_float = TRUE;
	params[c].is_char = FALSE;
	params[c++].u.fval = &names.change_lname;
...
char **argv;
{
	...
	char *optstring = "ac:fin:o:p:s:qz:I:U:";
...
fprintf(stdout, "\n");
	...
	if (!iflg) {
		...
		if (make_safety_checks()) {
		    ...
		    system("clear");
#endif
		    generate(seed, namesfile, outfile, inputfilename, first_uid, (comm == APPEND), quiet_flag, fixed_size);
	  ...
	  generate(seed, namesfile, outfile, inputfilename, first_uid, append_flag, quiet_flag, fixed_size);
	}
